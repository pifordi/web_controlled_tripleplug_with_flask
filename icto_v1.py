...
plug_status = {'a':['status', 'off'], 'b':['status', 'off'], 'c':['status', 'off']}
...
def control(name, turn_mode):
    ...
    plug_status.get(name)[1] = turn_mode
...
def home_turnall():
    ...
    return render_template('home.html', a=plug_status.get('a')[1], b=plug_status.get('b')[1], c=plug_status.get('c')[1])
...
def plugs_control():
    ...
    return render_template('plugs.html', plug=plug_status.get(plug)[1])
